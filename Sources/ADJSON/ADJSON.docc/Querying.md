...
A ``/ADJSONCore/JSONPointer`` addresses one location. Resolve it against a lazy ``/ADJSONCore/JSON``…

```swift
let title = doc.root[pointer: "/store/book/0/title"].string
```

…or against a ``/ADJSONCore/JSONValue``:
...
Pointer parsing throws a typed ``/ADJSONCore/JSONPointerError``. Array-index tokens follow RFC 6901 §4
exactly (`0` or `[1-9][0-9]*`; no leading `+`, sign, or zero-padding).

## JSONPath (RFC 9535)

A ``/ADJSONCore/JSONPath`` is compiled once to an AST and returns a nodelist in document order.
...
the U+2028/U+2029 line separators. Parse errors are typed ``/ADJSONCore/JSONPathError``.

## JSON Patch (RFC 6902)

A ``/ADJSONCore/JSONPatch`` is an ordered sequence of `add` / `remove` / `replace` / `move` / `copy` /
`test` operations applied to a ``/ADJSONCore/JSONValue``.
...
typed ``/ADJSONCore/JSONPatchError``.

## JSON Merge Patch (RFC 7396)

``/ADJSONCore/JSONMergePatch`` recursively merges an object patch into a target; a `null` member removes a
...
``/ADJSONCore/RelativeJSONPointer`` ascends a number of levels (with an optional `+N`/`-N` index
...
JSON Pointer access, the ``/ADJSONCore/JSONValue`` mutation primitives, and ``/ADJSONCore/JSONPatchError`` live
